...
export class TodosService {
  ...
  addTodo(item:Partial<Todo>):void{
    let list = this.jsonList();
     let id = list.reduce((e1:Json,e2:Json, index:number) => Math.max(e1.id, e2.id) == e1.id ? e1 : e2).id + 1;
    list.push({...{
      id:id,
      title: '',
      completed: false
    } as Json, ...item});
    sessionStorage.setItem(TodosService.TODO_KEY, JSON.stringify(list));
  }
  ...
  jsonList():Json[]{
    let value : string | null = sessionStorage.getItem(TodosService.TODO_KEY);
    if(value){
